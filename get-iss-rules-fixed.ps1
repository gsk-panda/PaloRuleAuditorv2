...
# Function to get security rules
function Get-SecurityRules {
    ...
    # Try a different API command approach - get device group specific config
    # Using the correct XML tag for device group name
    $cmd = "<show><config><panorama><device-group><name>$DeviceGroup</name></device-group></panorama></config></show>"
    $url = "$panoramaUrl/api/?type=op&cmd=$(UrlEncode($cmd))&key=$apiKey"
    Write-Host "API Command: $cmd"
    ...
    try {
        ...
        Write-Host "Has panorama: $($null -ne $xmlData.response.result.panorama)"
        Write-Host "Has device-group: $($null -ne $xmlData.response.result.panorama.'device-group')"
        
        # Try different XML paths to find the rules
        $entryPath = $null
        
        if ($xmlData.response.result.panorama.'device-group'.entry) {
            $entryPath = $xmlData.response.result.panorama.'device-group'.entry
            Write-Host "Found entry path via panorama.device-group.entry"
        } elseif ($xmlData.response.result.panorama.'device-group') {
            $entryPath = $xmlData.response.result.panorama.'device-group'
            Write-Host "Found entry path via panorama.device-group"
        }
        
        if ($entryPath) {
            # Check for pre-rulebase
            if ($entryPath.'pre-rulebase'.security.rules.entry) {
                $entries = $entryPath.'pre-rulebase'.security.rules.entry
                Write-Host "Found rules via pre-rulebase.security.rules.entry"
            } elseif ($entryPath.'pre-rulebase') {
                Write-Host "Found pre-rulebase but no rules entries"
            } else {
                Write-Host "No pre-rulebase found"
            }
            
            # If we found rule entries, process them
            if ($entries) {
                # Handle single entry vs array
                if ($entries -is [System.Xml.XmlElement]) {
                    $entries = @($entries)
            ...
            Write-Host "Could not find device group entry in XML"
        }
