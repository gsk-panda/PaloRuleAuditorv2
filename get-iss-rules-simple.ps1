...
function Get-SecurityRules {
    ...
    try {
        ...
        if ($xmlData.response.result.panorama.'device-group'.entry.'pre-rulebase'.security.rules.entry) {
            $entries = $xmlData.response.result.panorama.'device-group'.entry.'pre-rulebase'.security.rules.entry
