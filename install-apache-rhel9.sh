...
setup_application() {
    ...
    if command -v rsync &> /dev/null; then
        rsync -av --exclude=node_modules --exclude=dist "$source_dir/" "$APP_DIR/"
    else
        tar -cf - -C "$source_dir" --exclude=node_modules --exclude=dist . | tar -xf - -C "$APP_DIR"
    fi
...
main() {
    if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
        ...
        echo "  --backend-port N   Backend port (default: 3001)"
        exit 0
    fi
    while [[ $# -gt 0 ]]; do
        case "$1" in
